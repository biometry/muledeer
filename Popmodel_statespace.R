...
cat("
    ...
    sigma.proc ~ dunif(0.1, 0.2)           # Prior for sd of state process
    sigma2.proc <- pow(sigma.proc, 2)   # Variance = sd^2
    tau.proc <- pow(sigma.proc, -2)     # Tau = sqrt(sd)
    sigma.obs ~ dunif(0.1, 0.2)           # Prior for sd of observation process
...
# Initial values:
inits <- function(){list(sigma.proc = runif(1, 0.1, 0.2), mean.rmax = runif(1, 0.1, 10), K = runif(1,0.1,100), sigma.obs = runif(1, 0.1, 0.21), N.est = c(runif(1, 0.1, 10), rep(NA, (tsteps-1))))}
