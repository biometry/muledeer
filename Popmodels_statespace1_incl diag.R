...
png("StateSpace1_70000_140000.png", width=2200, height=1500)
par(mfrow=c(2,2),oma=c(5,5,5,5),mar=c(10, 10, 10, 10)) 
parinfo <- data.frame("0-1" = numeric(11), "0-2" = numeric(11), "0-3" = numeric(11), "0-4" = numeric(11), row.names=c("mean of rmax.mean","sd.rmax.mean","se.rmax.mean","mean of K", "sd.K", "se.K", "mean of sigma2.obs", "mean of sigma2.proc", "mean of sd.N.est","mean of se.N.est","MSS"))
...
for (i in 1:length(macrounits)){
  ...
  inits1 <- function(){list(sigma.proc = 1, mean.rmax = 0.3, K = 2, sigma.obs = 1, N.est = c(1, rep(NA, (tsteps-1))))
                       list(sigma.proc = 1, mean.rmax = 5, K = 50, sigma.obs = 1, N.est = c(1, rep(NA, (tsteps-1))))
                       list(sigma.proc = 1, mean.rmax = 3, K = 25, sigma.obs = 1, N.est = c(1, rep(NA, (tsteps-1))))} 
  
  
  # Compile the model and run MCMC for burn-in phase:
  model_fit <- jags.model(file = "model_ss.jags", data = model.data, inits = inits, n.chains = 3, n.adapt = 70000)
  ...
  result_sss <- coda.samples(model = model_fit, variable.names = parameters, n.iter = 140000)
  ...
  parinfo[,i] <- format(c(mean.rmax_mean[,1], mean.rmax_mean[,2],mean.rmax_mean[,4],K_mean[,1],K_mean[,2],K_mean[,4],sigmas_mean[,1], mean(N.est_mean[,2]),mean(N.est_mean[,4]),MSE), scientific=FALSE)
}
...
for (k in c(1,4,7,10)){
  m<- m+1
  name <- paste(c("Diag_StateSpace1_60000_12000_",m,".png"),collapse="")
...
name2 <-paste(c("Diag_StateSpace1_70000_140000_GelmanDiag",".txt"),collapse="")
