...
gam_denspred <- data.frame(year=AllMeans$year, macrounit=AllMeans$macrounit, AvrgWinterMinTemp=AllMeans$AvrgWinterMinTemp)
gam_denspred <- cbind(gam_denspred, predict(gam_dens, se.fit=T, newdata=data.frame("year"=AllMeans$year, "macrounit"=AllMeans$macrounit, "AvrgWinterMinTemp"=AllMeans$AvrgWinterMinTemp), type="response"))
macrounitplots(glmobject = gam_denspred,title="gam_dens fall - effect of Mean Winter Minimum Temperature",colour="red")
...
gam_temp <- gam(RepRateFall_mean ~ s(year, bs="cs") +s(AvrgWinterMinTemp, bs="cs"), data=AllMeans)#1
gam_temp <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(AvrgWinterMinTemp, by=macrounit, bs="cs") + macrounit, data=AllMeans)#2
gam_temp <- gam(RepRateFall_mean ~ s(year, by=macrounit, bs="cs") +s(AvrgWinterMinTemp, bs="cs") + macrounit, data=AllMeans)#3
...
plot(gam_tempres ~AllMeans$year[which(!is.na(AllMeans$RepRateFall_mean))]) #
...
gam_hunt <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(HuntDen_All_mean, bs="cs"), data=AllMeans)#1
gam_hunt <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(HuntDen_All_mean, by=macrounit, bs="cs") + macrounit, data=AllMeans)#2
gam_hunt <- gam(RepRateFall_mean ~ s(year, by=macrounit, bs="cs") + s(HuntDen_All_mean, bs="cs") + macrounit, data=AllMeans)#3
...
plot(gam_huntres ~AllMeans$year[which(!is.na(AllMeans$RepRateFall_mean))]) #
...
gam_oil <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(WellDen_mean, bs="cs"), data=AllMeans)#1
gam_oil <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(WellDen_mean, by=macrounit, bs="cs") + macrounit, data=AllMeans)#2
gam_oil <- gam(RepRateFall_mean ~ s(year, by=macrounit, bs="cs") + s(WellDen_mean, bs="cs") + macrounit, data=AllMeans)#3
...
plot(gam_oilres ~AllMeans$year[which(!is.na(AllMeans$RepRateFall_mean))]) #
...
gam_coyote <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(CoyoteDen_mean, bs="cs"), data=AllMeans)
gam_coyote <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(CoyoteDen_mean, by=macrounit, bs="cs") + macrounit, data=AllMeans)
gam_coyote <- gam(RepRateFall_mean ~ s(year, by=macrounit, bs="cs") + s(CoyoteDen_mean, bs="cs") + macrounit, data=AllMeans)
...
plot(gam_coyoteres ~AllMeans$year[which(!is.na(AllMeans$RepRateFall_mean))]) #
...
gam_woodyveg <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(WoodyVeg_mean, bs="cs"), data=AllMeans)
gam_woodyveg <- gam(RepRateFall_mean ~ s(year, bs="cs") + s(WoodyVeg_mean, by=macrounit, bs="cs") + macrounit, data=AllMeans)
gam_woodyveg <- gam(RepRateFall_mean ~ s(year, by=macrounit, bs="cs") + s(WoodyVeg_mean, bs="cs") + macrounit, data=AllMeans)

#correlation issues in some of the MUs,so:
gam_woodyveg <- gam(RepRateFall_mean ~ s(WoodyVeg_mean, bs="cs"), data=AllMeans)
gam_woodyveg <- gam(RepRateFall_mean ~ s(WoodyVeg_mean, by=macrounit, bs="cs") + macrounit, data=AllMeans)
...
plot(gam_woodyvegres ~AllMeans$year[which(!is.na(AllMeans$RepRateFall_mean))]) #
